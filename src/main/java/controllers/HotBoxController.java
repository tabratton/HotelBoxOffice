...
/**
 * main.hotelbox.HotelBox controller class for the entire layout.
 *
...
public class HotBoxController implements Initializable {
  ...
  public void loadSearchResults() {
    var currentSearchText = searchBox.getText().toLowerCase().trim();
    if (currentSearchText.length() == 0 || currentSearchText.matches("\\s+")) {
      var alert = new Alert(Alert.AlertType.ERROR);
      alert.setTitle("Empty Search");
      alert.setHeaderText(null);
      alert.setContentText("You must enter something besides spaces in your search.");
      alert.showAndWait();
      searchBox.setText("");
    } else {
  ...
  private void addSearchTerm(String term) {
    HotelBox.dbConnection.saveSearchTerm(term);
  }
}