...
-- Additions to Customer table
ALTER TABLE HotelBoxOffice.CUSTOMER ADD CUSTOMER_ADMIN BOOLEAN DEFAULT FALSE  NOT NULL;
ALTER TABLE HotelBoxOffice.CUSTOMER ADD CUSTOMER_ADDRESS VARCHAR(40) NOT NULL;
ALTER TABLE HotelBoxOffice.CUSTOMER ADD CUSTOMER_CITY VARCHAR(40) NOT NULL;
ALTER TABLE HotelBoxOffice.CUSTOMER ADD CUSTOMER_ZIPCODE VARCHAR(5) NOT NULL;

-- Additions to Movie table
ALTER TABLE HotelBoxOffice.MOVIES ADD MOVIE_PRICE DECIMAL(6,2) NOT NULL;
ALTER TABLE HotelBoxOffice.MOVIES ADD TIMES_VIEWED INT DEFAULT 0 NOT NULL;

-- Added Search term table, keeps list of all searched terms and their
-- frequencies
CREATE TABLE HotelBoxOffice.SEARCH_TERMS
(
  SEARCHTERM_ID INT PRIMARY KEY NOT NULL AUTO_INCREMENT,
  TERM VARCHAR(40) NOT NULL,
  FREQUENCY INT DEFAULT 1 NOT NULL
);
CREATE UNIQUE INDEX SEARCH_TERMS_TERM_uindex ON HotelBoxOffice.SEARCH_TERMS (TERM);
CREATE UNIQUE INDEX SEARCH_TERMS_SEARCHTERM_ID_uindex ON HotelBoxOffice.SEARCH_TERMS (SEARCHTERM_ID);

-- Add times viewed to actors also
ALTER TABLE HotelBoxOffice.ACTORS ADD TIMES_VIEWED INT DEFAULT 0 NOT NULL;
-- Make RENTAL_ID auto inc
ALTER TABLE HotelBoxOffice.CUSTOMER_RENTALS MODIFY RENTAL_ID INT(11) unsigned NOT NULL AUTO_INCREMENT;
-- Add on update/delete cascade to all foreign keys
ALTER TABLE HotelBoxOffice.CASTING DROP FOREIGN KEY CASTING_ACTOR_ID_fk;
ALTER TABLE HotelBoxOffice.CASTING
ADD CONSTRAINT CASTING_ACTOR_ID_fk
FOREIGN KEY (ACTOR_ID) REFERENCES ACTORS (ACTOR_ID) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE HotelBoxOffice.CASTING DROP FOREIGN KEY CASTING_MOVIES_MOIVE_ID_fk;
ALTER TABLE HotelBoxOffice.CASTING
ADD CONSTRAINT CASTING_MOVIES_MOIVE_ID_fk
FOREIGN KEY (MOVIE_ID) REFERENCES MOVIES (MOVIE_ID) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE HotelBoxOffice.CUSTOMER_RENTALS DROP FOREIGN KEY CUSTOMER_RENTALS_CUSTOMER_CUSTOMER_ID_fk;
ALTER TABLE HotelBoxOffice.CUSTOMER_RENTALS
ADD CONSTRAINT CUSTOMER_RENTALS_CUSTOMER_CUSTOMER_ID_fk
FOREIGN KEY (CUSTOMER_ID) REFERENCES CUSTOMER (CUSTOMER_ID) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE HotelBoxOffice.CUSTOMER_RENTALS DROP FOREIGN KEY CUSTOMER_RENTALS_MOVIES_MOIVE_ID_fk;
ALTER TABLE HotelBoxOffice.CUSTOMER_RENTALS
ADD CONSTRAINT CUSTOMER_RENTALS_MOVIES_MOIVE_ID_fk
FOREIGN KEY (MOVIE_ID) REFERENCES MOVIES (MOVIE_ID) ON DELETE CASCADE ON UPDATE CASCADE;